...
contract FarmerDogeLocker is Ownable{
    ...
    function harvestAll() internal {

        if(distributors.length == 0) return;

        for(uint i = 0; i < distributors.length; i++) {
            _harvest(distributors[i]);
            Distribution storage _distribution = distributions[distributors[i]];
            _distribution.reflectionDebt = 0;
        }
    }
    ...
    function _harvest(address user) internal {
        _updatePool();
        Distribution storage _distribution = distributions[user];
        uint256 amount = tokenSupply.mul(_distribution.alloc).div(10000).sub(_distribution.tokenDebt);
        ...
        if(pending > 0) {
            IERC20(reflectionToken).safeTransfer(user, pending);
            allocatedReflections = allocatedReflections.sub(pending);
        }
    }

    function transferTokenOut(address tokenOut) public onlyOwner {
    ...
    function setDividendToken(address newDividendToken) external onlyOwner {
        require(newDividendToken != reflectionToken, "FarmerDogeLocker: Cannot update dividend token to same value");
        require(newDividendToken != address(0), "FarmerDogeLocker: The dividend token cannot be 0");
        require(newDividendToken != address(this), "FarmerDogeLocker: The dividend token cannot be set to the current contract");
        ...
        if(amount > 0){
            ...
            transferTokenOut(reflectionToken);
            accReflectionPerShare = 0;
            allocatedReflections = 0;
        }
        reflectionToken = newDividendToken;
        emit DividendTokenChange(newDividendToken, reflectionToken);
    }
